...
function renderTabel(kelas){
  const container = document.getElementById("kelas-container"); container.innerHTML="";
...
function unduhCSV(){
  ...
  const a=document.createElement("a"); a.href=url; a.download="absensi_"+userKelas+".csv"; a.click();
...
function tambahSiswa(){
  ...
  dataKelas[kelas].push({nama,gender}); localStorage.setItem("dataKelas",JSON.stringify(dataKelas));
...
function tambahGuru(){
  ...
  akunGuru.push({username:user,password:pass,kelas}); localStorage.setItem("akunGuru",JSON.stringify(akunGuru)); renderGuruList();
  alert("✅ Akun guru ditambahkan!");
}
function hapusGuru(i){ akunGuru.splice(i,1); localStorage.setItem("akunGuru",JSON.stringify(akunGuru)); renderGuruList();}
function resetPass(i){ akunGuru[i].password="1234"; localStorage.setItem("akunGuru",JSON.stringify(akunGuru)); renderGuruList(); alert("✅ Password direset ke '1234'");}
function renderGuruList(){
  const ul=document.getElementById("daftarGuru"); ul.innerHTML="";
  akunGuru.forEach((g,i)=>{ul.innerHTML+=`<li>${g.username} (${g.kelas}) 
...
function uploadTemplate(){
  const fileInput=document.getElementById("uploadFile"); if(!fileInput.files.length){alert("❌ Pilih file CSV terlebih dahulu!");return;}
  const file=fileInput.files[0]; const reader=new FileReader();
  reader.onload=function(e){
    const text=e.target.result; const lines=text.trim().split("\n");
    for(let i=1;i<lines.length;i++){
      const [nama,kelas]=lines[i].split(",");
      if(nama&&kelas&&dataKelas[kelas]) dataKelas[kelas].push({nama:nama.trim(),gender:"L"});
    }
    localStorage.setItem("dataKelas",JSON.stringify(dataKelas));
    alert("✅ Siswa berhasil ditambahkan dari template!"); fileInput.value="";
  };
...
function tampilkanGrafik(data,judul){
  ...
  const ctxPie=document.getElementById("pieChart"); const ctxBar=document.getElementById("barChart");
  ...
  data.filter(d=>new Date(d.tanggal).getMonth()===bulanIni).forEach(d=>countStatus[d.status]++);
  pieChart=new Chart(ctxPie,{type:"pie",data:{labels:Object.keys(countStatus),datasets:[{data:Object.values(countStatus),backgroundColor:["#22c55e","#f59e0b","#3b82f6","#ef4444","#8b5cf6"]}]},options:{plugins:{title:{display:true,text:`Rekap Bulan Ini (${judul})`}}}});
  let perBulan=Array(12).fill(0);
  data.forEach(d=>{perBulan[new Date(d.tanggal).getMonth()]++;});
  barChart=new Chart(ctxBar,{type:"bar",data:{labels:["Jan","Feb","Mar","Apr","Mei","Jun","Jul","Agu","Sep","Okt","Nov","Des"],datasets:[{label:"Jumlah Absensi",data:perBulan,backgroundColor:"#2563eb"}]},options:{plugins:{title:{display:true,text:`Rekap Tahunan (${judul})`}}}});
}
function tampilkanGrafikGuru(){tampilkanGrafik(absensiLog.filter(l=>l.kelas===userKelas),userKelas);}
function tampilkanGrafikAdmin(){let kelas=document.getElementById("kelasGrafik").value; let data=absensiLog; if(kelas!=="Semua") data=data.filter(l=>l.kelas===kelas); tampilkanGrafik(data,kelas);}
